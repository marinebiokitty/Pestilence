...
\NEW{PC}{\cPlaguebearer}{
	...
	\s\MYnumber	{69752}
	\s\MYdesc	{a skinny urchin in mismatched clothes}
	\s\MYsex	{\female}
	\s\MYplayer	{}
...
\NEW{PC}{\cApprentice}{
	\s\MYname	{Alexandra Kain}
	\s\MYfile	{Apprentice.tex}
	\s\MYnumber	{24873}
	\s\MYdesc	{a tall, severe woman with dark hair}
	\s\MYsex	{\female}
	\s\MYplayer	{}
...
\NEW{PC}{\cShaman}{
	\s\MYname	{Katerina Malinov}
	\s\MYfile	{Shaman.tex}
	\s\MYnumber	{84785}
	\s\MYdesc	{a friendly older woman with a soothing smile}
	\s\MYsex	{\female}
	\s\MYplayer	{}
...
\NEW{PC}{\cCon}{
	\s\MYname	{Nadya Saburov}
	\s\MYfile	{ConA.tex}
	\s\MYnumber	{31479}
	\s\MYdesc	{a charming young woman with a scar on her brow}
	\s\MYsex	{\female}
	\s\MYplayer	{}
...
%NPC character. Real name: Mikhail Lomnosov. Because why not.
\NEW{PC}{\cGhost}{
	   \s\MYname	{Angry Ghost}
	   \s\MYfile	{Ghost.tex}
	   \s\MYsex {\male}
	   \s\MYnumber	{0016}
	   \s\MYdesc     {A vaguely human-shaped figure with red eyes and grey skin}
		 \s\MYplayer	{}
	  \s\MYblues	{}
	  \s\MYgreens	{}
	  \s\MYabils	{} 
